...
/// @title ICompanyFactory — Company Factory Interface
/// @notice Interface of the CompanyRegistry contract in its factory role:
///         deploys one standalone Company per company, routes claims, and
///         maintains the employee → Company routing index.
/// @dev Inherits ICompanyRegistry so onClaimed (the claim callback called by
///      NFT57B) is part of the factory interface via inheritance. The public
///      immutable getters bonusReward/recognitionToken are informational —
///      used at deploy time to verify the factory was wired to the right
///      reward contracts.
interface ICompanyFactory is ICompanyRegistry {
